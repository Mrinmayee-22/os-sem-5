...
void lruPageReplacement(int pages[], int numPages, int numFrames)
{
    ...
    int pageFaults = 0;
    int pageHits = 0;
    int recent[numFrames]; // To track usage (timestamps)

    // Initialize all frames to -1 (empty) and recent to -1
    for (int i = 0; i < numFrames; i++)
    {
        frames[i] = -1;
        recent[i] = -1;
    ...
    for (int i = 0; i < numPages; i++)
    {
        ...
        int idx = isPagePresent(currentPage, frames, numFrames);
        if (idx != -1)
        {
            // Page hit
            pageHits++;
            recent[idx] = i; // Update usage timestamp
